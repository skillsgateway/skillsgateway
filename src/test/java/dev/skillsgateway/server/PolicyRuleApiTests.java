...
class PolicyRuleApiTests extends AbstractGatewayTest {
    ...
    void rule_lifecycle_is_compiled_gated_and_audited() throws Exception {
        ...
        try {
            ...
            mockMvc.perform(get("/api/v1/policy/rules").with(oidcLogin().idToken(token -> token.subject("root"))))
                    .andExpect(status().isOk())
            ...
            mockMvc.perform(put("/api/v1/policy/rules/{name}", name)
                            .with(oidcLogin().idToken(token -> token.subject("root")))
                            .contentType(MediaType.APPLICATION_JSON)
            ...
            mockMvc.perform(put("/api/v1/policy/rules/{name}", "no-such-rule")
                            .with(oidcLogin().idToken(token -> token.subject("root")))
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(ruleJson("no-such-rule", "true", true)))
                    .andExpect(status().isNotFound());
            mockMvc.perform(delete("/api/v1/policy/rules/{name}", "no-such-rule")
                            .with(oidcLogin().idToken(token -> token.subject("root"))))
                    .andExpect(status().isNotFound());
