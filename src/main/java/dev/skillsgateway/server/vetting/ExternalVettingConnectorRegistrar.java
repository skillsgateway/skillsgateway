 ...
 * per entry (GW_VETTING_0024). Registering them as ordinary beans means they join the chain through the
 * same {@code List<Vetter>} injection the built-ins do — {@code VettingService} is
 ...
 * producing a chain that silently drops or shadows a vetter:
 *
 * <ul>
 ...
 *       ExternalConnectorProperties}), because two vetters sharing a name break the continuity of
...
public class ExternalVettingConnectorRegistrar implements ImportBeanDefinitionRegistrar, EnvironmentAware {
    ...
    /** Configuration prefix the external vetters are bound from. */
