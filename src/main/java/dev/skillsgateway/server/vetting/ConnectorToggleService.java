...
/**
 * The administrative connector on/off switch (GW_VETTING_0029), and the only place its resolution rule
...
public class ConnectorToggleService {
    ...
    /** Ledger event when a connector is switched off (GW_VETTING_0029). */
    public static final String EVENT_DISABLED = "connector-disabled";

    /** Ledger event when a connector is switched back on (GW_VETTING_0029). */
    ...
    /**
     * Whether a connector runs for one marketplace's chain run (GW_VETTING_0029): the per-marketplace
     * setting if there is one, otherwise the global setting, otherwise enabled.
     */
    @Requirements({"GW_VETTING_0029"})
