 ...
 * side-effect free with respect to gateway state — the only thing a vetter may do is read the
 * snapshot it was handed and return a verdict.
 *
 * <p>A vetter is allowed to throw and allowed to be slow: {@link VettingService} records either
 * as an {@link VerdictState#ERROR} verdict, which blocks. It is never allowed to be skipped.
 */
public interface Vetter {
    ...
    /**
     * Identity of the rule set this vetter currently carries, recorded on every run it takes
     ...
     * vetter? Bump it whenever the rules change.
     *
     * <p>Default {@code "1"} so that adding a vetter never requires thinking about versioning
