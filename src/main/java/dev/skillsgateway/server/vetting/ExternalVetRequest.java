...
/**
 * The request body the gateway POSTs to an external vetting connector (GW_VETTING_0024): the snapshot's
 * identity and a bundle of its scannable file content, the same view of the snapshot a built-in
 * vetter walks. Quarantined content is never served through the facade, so the external service
