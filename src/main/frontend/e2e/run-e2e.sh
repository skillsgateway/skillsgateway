...
# whole suite passes only if claim-to-role mapping works end to end through a real login (GW_AUTH_0015).
#
...
# start (GW_AUTH_0026), so the claim mapping below is what satisfies the bootstrap check as well as what
# the role assertions exercise: remove it and the gateway will not boot at all.
#
# The name-collision rule is off: the specs register the same fixture repeatedly as marketplaces of
# their own, so every approval after the first would meet a "hello" another marketplace serves. The
# rule is exercised by the Java suite (AbstractNameCollisionTest) and its portal notice by stories.
...
SKILLSGATEWAY_APPROVAL_NAMECOLLISION_ENABLED=false \
