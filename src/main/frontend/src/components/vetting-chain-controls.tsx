...
import { sourceWord, type FlowNode } from "@/lib/vetting-flow";

/**
 * The three chain controls, written once for both scopes they exist in.
 *
 * A control here is scoped by `marketplace`: a name writes the setting for that marketplace, and its
 * absence writes the global one — which is the server's own contract for these endpoints, not a
 * convention invented here. The estate-wide page and the marketplace card are therefore the same
 * controls with the same keyboard behaviour, the same live region and the same audit hint; only the
 * words that name the scope differ.
 *
 * @Requirements GW_VETTING_0034
 * @Requirements GW_VETTING_0035
 */

/** A scope, said the way a sentence about a change needs it. */
export function scopePhrase(marketplace: string | undefined): string {
  return marketplace ?? "every marketplace with no override of its own";
}

/** The same, short enough for a control's accessible name. */
function scopeName(marketplace: string | undefined): string {
  return marketplace ?? "the default chain";
}
...
export function ToggleControls({
  ...
  const scope = scopeName(marketplace);
  ...
  return (
    <div className="mt-2 space-y-2 rounded-md border bg-muted/40 p-3">
      ...
      <p id={hintId} className="text-xs text-muted-foreground">
        {marketplace
          ? "The change is scoped to this marketplace and overrides the global setting."
          : "The change is the global setting. It applies to " +
            scopePhrase(marketplace) +
            "; a marketplace that overrides this vetter keeps its own setting."}{" "}
        It is recorded on the audit ledger with your identity, the vetter, the scope, the new state
        and this note.
      </p>
      <Button
        size="sm"
        ...
        aria-label={`${next ? "Enable" : "Disable"} ${vetter} for ${scope}`}
        onClick={() =>
          toggle.mutate(
            { vetter, marketplace, enabled: next, reason: reason.trim() || undefined },
            {
              onSuccess: () => {
                toast.success(`${vetter} ${next ? "enabled" : "disabled"} for ${scope}`);
                setReason("");
              },
  ...
  return (
    <div className="space-y-2 rounded-md border bg-muted/40 p-3">
              ...
              onSuccess: () => {
                toast.success(
                  next === "stop-after-fail"
                    ? `The chain will stop at the first failure for ${scope}`
                    : `Every vetter will run for ${scope}`,
                );
                setReason("");
              },
              onError: (error) => toast.error(error.message),
      ...
      <p id={hintId} className="text-xs text-muted-foreground">
        ...
        after the finding that stopped it is waived.{" "}
        {marketplace
          ? `Currently ${sourceWord(settings.modeSource)}.`
          : `This is the mode for ${scopePhrase(marketplace)}. Currently ${sourceWord(settings.modeSource)}.`}
      </p>
      <div className="space-y-1">
        ...
        <Input
          ...
          onChange={(event) => setReason(event.target.value)}
          placeholder={
            marketplace ? "Why should this marketplace stop early?" : "Why should the estate stop early?"
          }
        />
      </div>
    </div>
    ...
    <div className="space-y-2 rounded-md border bg-muted/40 p-3">
      ...
      */}
      ...
      <p id="chain-order-hint" className="text-xs text-muted-foreground">
        ...
        look at all, so the cheap and deterministic ones belong first.{" "}
        {marketplace ? "" : `This is the order for ${scopePhrase(marketplace)}. `}Currently{" "}
        {sourceWord(settings.orderSource)}.
      </p>
      <ol aria-describedby="chain-order-hint" aria-label={`Vetter order of ${scope}`}>
        ...
        <Button
          ...
          onClick={() =>
            setOrder.mutate(
              ...
              {
                onSuccess: () => {
                  toast.success(`Vetter order saved for ${scope}`);
                  onPending(null);
                },
                onError: (error) => toast.error(error.message),
              },
...
export function cssId(vetter: string): string {
...
export function reorder(nodes: FlowNode[], order: string[]): FlowNode[] {
