...
- [x] 1.1 Add GW_0163 (the closure as an immutable domain object) and GW_0164
      (approval requires a complete closure) to `docs/reqstool/requirements.yml`
- [x] 1.2 Add SVC_GW_0163 and SVC_GW_0164 (GIVEN/WHEN/THEN) to
      `docs/reqstool/software_verification_cases.yml`
- [x] 1.3 Spec deltas: `marketplace-ingestion` (GW_0163), `snapshot-approval`
      (GW_0164)

## 2. Tests (red before green)

- [x] 2.1 `SnapshotClosureDigestTests` (`@SVCs({"SVC_GW_0163"})`), pure unit:
...
- [x] 2.2 `SnapshotClosureTests` (`@SVCs({"SVC_GW_0163"})`), the enabled Spring
...
- [x] 2.3 `ClosureCompletenessTests` (`@SVCs({"SVC_GW_0164"})`): each tampering
