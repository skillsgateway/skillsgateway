...
- The reqstool JSON schema shipped with reqstool 0.12.1 already supports both:
  ...
  plain strings with no character restriction, so `GW_INGEST_0026.1` and
  `SVC_GW_INGEST_0026.1` are valid without any schema or tooling change.
...
GW_INGEST_0026 — *Resource-bounded source resolution* is unusual among the candidates in
...
| --- | --- |
| `GW_INGEST_0026.1` — a bound on the bytes one source may send | `max-received-bytes` |
| `GW_INGEST_0026.2` — a bound on one source's decompressed size | `max-inflated-bytes` |
| `GW_INGEST_0026.3` — a bound on the decompressed size of a manifest's sources together | `max-closure-bytes` |
| `GW_INGEST_0026.4` — a bound on how far a source's content may expand | `max-inflation-ratio` |
| `GW_INGEST_0026.5` — a bound on the number of objects one source contributes | `max-objects` |
| `GW_INGEST_0026.6` — a bound on the largest single file one source contributes | `max-blob-bytes` |
| `GW_INGEST_0026.7` — a bound on the depth of a source's directory tree | `max-tree-depth` |
| `GW_INGEST_0026.8` — a bound on the time a whole resolution may take | `deadline` |

`max-redirects` is the ninth field of that record and is deliberately **not** a
child here: it belongs to GW_INGEST_0025 — *Address, redirect and transport policy for
...
Why not finer: nothing here nests. `GW_INGEST_0026.4` carries both the ratio and the
...
Both are verified by `SVC_GW_INGEST_0026` against two existing tests. The parent SVC is
not a rubber stamp.
...
`SVC_GW_INGEST_0026` is instead annotated on the two `ResolutionBudgetTests` methods
...
| --- | --- | --- |
| `ExternalSourceResolver.resolve` | `GW_INGEST_0023, GW_INGEST_0025, GW_INGEST_0026` | unchanged — it is the parent capability: it stops resolving and refuses the ingestion |
| `ResolutionBudget.accept` | `GW_INGEST_0026` | `GW_INGEST_0026, GW_INGEST_0026.3` — composes the named violation, and accumulates the manifest total |
| `ResolutionBudget.reason` | (none) | `GW_INGEST_0026.1, .2, .4, .5, .6, .7` — the per-source checks |
| `ResolutionBudget.maxReceivedBytes` | (none) | `GW_INGEST_0026.1` — published so the transfer can stop at it |
| `ResolutionBudget.expired` | `GW_INGEST_0026` | `GW_INGEST_0026.8` |
| `GuardedHttpConnectionFactory.create` | `GW_INGEST_0025, GW_INGEST_0026` | `GW_INGEST_0025, GW_INGEST_0026.1` |
| `GuardedHttpConnectionFactory.Bounded.count` | (none) | `GW_INGEST_0026.1` — where the transfer is actually aborted |

`@SVCs`: the twelve `ResolutionBudgetTests` methods and the two `GW_INGEST_0026` tests
in `ExternalSourceResolutionTests` are redistributed one bound per child, with
the two cross-cutting tests keeping `SVC_GW_INGEST_0026`. No test was added, removed,
weakened or renamed.
...
**1. GW_INGEST_0030 — *The resolved closure is recorded as an immutable domain object of
the snapshot* (1475 chars). Decompose — highest value after GW_INGEST_0026.** Its
...
**2. GW_INGEST_0024 — *Deterministic composite snapshot with a gateway-local manifest*
...
**3. GW_INGEST_0025 — *Address, redirect and transport policy for source resolution*
...
Recommended order for the next changes: **GW_INGEST_0030, then GW_INGEST_0024.** Leave GW_INGEST_0025.
...
- **Merge conflicts.** Several branches append new ids to the end of
  ...
  change edits the GW_INGEST_0026 block in the middle of both. Resolution is the usual
