...
(`GW_INGEST_0026.1`, `SVC_GW_INGEST_0026.1`).
...
1. **Verification granularity.** One id gets one SVC. GW_INGEST_0026 — *Resource-bounded
...
its annotations and SVCs moved in the same commit, starting with GW_INGEST_0026 as the
worked example that establishes the pattern for the rest" — and that is what this
is.

- **GW_INGEST_0026 — *Resource-bounded source resolution* keeps its id and gains eight
  children**, `GW_INGEST_0026.1` through `GW_INGEST_0026.8`, each linked to it by
...
- **The parent's own text becomes what actually holds across the whole set** —
  ...
  a placeholder.
- **One parent SVC and eight child SVCs.** `SVC_GW_INGEST_0026` keeps its id and now
  verifies the cross-cutting contract; `SVC_GW_INGEST_0026.1` – `SVC_GW_INGEST_0026.8` each
  verify one bound.
...
- The old text bounded *"the bytes it will accept in total across a manifest's
  ...
  accumulated received bound is wanted, it is a new child (`GW_INGEST_0026.9`) and a new
...
- The old text said the refusal happens *"without first materialising the content
  ...
  that is already in quarantine. The clause now lives on `GW_INGEST_0026.1`, where it
...
- `marketplace-ingestion`: GW_INGEST_0026 — *Resource-bounded source resolution* is now
  a parent with eight children. No behaviour delta.

## Out of scope

- **The other candidates in #299 are not touched.** The issue lists eight; this
  change does one. See `design.md` for the inspection of GW_INGEST_0030, GW_INGEST_0024 and
  GW_INGEST_0025 and the priority order for whoever picks up the next one — including
  the finding that GW_INGEST_0025 should largely stay monolithic.
