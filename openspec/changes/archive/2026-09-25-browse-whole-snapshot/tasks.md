...
- [x] 3.1 `queries.ts`: infinite queries for the tree, search and diff pages;
  the single-path diff; the decision invalidates the preview reads. Verify with
  `pnpm verify`.
- [x] 3.2 `SnapshotFileTree` and `SnapshotExplorer`: folders load on open, with
...
- [x] 3.3 The Diff tab's "Files changed against the served commit", paged with
  inline diffs; the delta line reads `summary`. Verify with component tests and
  a `snapshot-delta` unit test.
- [x] 3.4 Remove the now-unused client tree building from
  `lib/snapshot-tree.ts` and its unit tests. None of them is an SVC test.
- [x] 3.5 Run `/impeccable audit` and `/impeccable harden` on the explorer and
  the Diff tab. Fix each finding, or dismiss it in the PR body.
- [x] 3.6 The e2e explorer spec passes unchanged, and a new e2e assertion for
  the Diff tab's file list is tagged `SVC_GW_APPROVAL_0028`.

## 4. Documentation

- [x] 4.1 Update `reference/api/marketplaces.md`, `reference/portal.md` and
