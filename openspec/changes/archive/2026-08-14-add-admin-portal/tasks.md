...
- [x] 1.1 Add GW_INGEST_0007/GW_AUTH_0005 + SVC_GW_INGEST_0007/SVC_GW_AUTH_0005 to the SSOT
...
## 3. Portal implementation (GW_INGEST_0007, GW_AUTH_0005)

- [x] 3.1 App shell: layout, nav, session/user surface, error + loading states
- [x] 3.2 Marketplaces page: list, register (client-side Zod validation), ingest action, @Requirements({"GW_INGEST_0007"})
- [x] 3.3 Snapshots: states, approve/reject actions, provenance view
- [x] 3.4 Audit page: fetch ledger table
- [x] 3.5 Tokens page: create with show-once cleartext, revoke, @Requirements({"GW_AUTH_0005"})
...
## 5. E2E (SVC_GW_INGEST_0007, SVC_GW_AUTH_0005)

- [x] 5.1 compose.e2e.yaml: PostgreSQL + navikt/mock-oauth2-server
- [x] 5.2 Playwright specs with real OIDC login: register→ingest→approve workflow (@SVCs SVC_GW_INGEST_0007), token create/revoke show-once (@SVCs SVC_GW_AUTH_0005)
...
- [x] 7.3 GW_INGEST_0008 snapshot content inventory: /api/snapshots/{id}/content + SnapshotContentService + SVC_GW_INGEST_0008 test; detail page shows plugins/skills
- [x] 7.4 GW_INGEST_0009 forge metadata (GitHub/GitLab/Gitea, best effort): V2 migration, ForgeMetadataService, SVC_GW_INGEST_0009 test with fake forge
...
- [x] 7.8 GW_AUDIT_0002 admin-action audit: register/ingest/approve/reject/token events appended to the ledger with the acting identity; SVC_GW_AUDIT_0002 test
