...
- [x] 1.1 Read `common-release-tag.yml` at the pinned SHA and record what it does
      ...
      creating anything.
- [x] 1.2 Confirm `SVC_GW_RELEASE_0003`'s existing assertions encode the old order
...
## 2. Reorder (GW_RELEASE_0003 / SVC_GW_RELEASE_0003)
...
- [x] 2.3 Move the `approve` job above `tag` in the file so it reads in
      ...
      rationale.
- [x] 2.4 `SVC_GW_RELEASE_0003` asserts the approval precedes the tag and that every
      publisher waits on the tag.

## 3. Report a partial release (GW_RELEASE_0003 / SVC_GW_RELEASE_0003)
...
- [x] 3.3 `SVC_GW_RELEASE_0003` asserts the job exists, depends on `tag` and `promote`,
      and survives a cancelled run.

## 4. Requirements and documentation

- [x] 4.1 `GW_RELEASE_0003` names the tag among the steps the approval precedes and gains
      the report obligation; revision `0.2.0`.
- [x] 4.2 `SVC_GW_RELEASE_0003` gains the matching assertions; revision `0.2.0`.
