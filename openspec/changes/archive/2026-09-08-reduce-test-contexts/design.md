...
| `AdoptionTests`, `FourEyesTests`, `MachineApiScopeTests`, `MachineCredentialAdminTests`, `MachineCredentialLifecycleTests`, `MachineRoleIntersectionTests` each declared their own `skills-gateway.roles.admins` | one shared `AbstractNamedAdminsTest` naming the union | Each declared it only because authorization is always enforced (GW_0138 — Authorization on the web surface is always enforced) and a gateway with no administrator refuses to start (GW_0139 — A gateway with no administrator refuses to start). Which *names* are in the list is not something any of their assertions reads. |
