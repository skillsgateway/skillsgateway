...
- [x] 2.1 Extend `.github/workflows/native.yml` trigger with `tags: ['v*']` and
      add job permissions `packages: write`, `id-token: write`,
      `attestations: write` (keep `contents: read`).
- [x] 2.2 Add publish steps gated on `github.event_name == 'push'`: GHCR login
...
- [x] 2.3 Write the pushed digest and tags to `$GITHUB_STEP_SUMMARY`.
- [x] 2.4 Attest the SBOM (`actions/attest-sbom`) against the pushed digest,
...
- [x] 4.1 Update `docs/manual/` (installation/reference): image coordinates
...
- [x] 5.1 Run all five gates fresh after the last edit; write
      `openspec/changes/publish-container-image/evidence.md` with command tails
      and commit SHA.
- [x] 5.2 Sync the delta into `openspec/specs/release-packaging/spec.md` and
