...
- [x] 1.1 Add GW_0164 (the closure as an immutable domain object) and GW_0165
      (approval requires a complete closure) to `docs/reqstool/requirements.yml`
- [x] 1.2 Add SVC_GW_0164 and SVC_GW_0165 (GIVEN/WHEN/THEN) to
      `docs/reqstool/software_verification_cases.yml`
- [x] 1.3 Spec deltas: `marketplace-ingestion` (GW_0164), `snapshot-approval`
      (GW_0165)

## 2. Tests (red before green)

- [x] 2.1 `SnapshotClosureDigestTests` (`@SVCs({"SVC_GW_0164"})`), pure unit:
...
- [x] 2.2 `SnapshotClosureTests` (`@SVCs({"SVC_GW_0164"})`), the enabled Spring
...
- [x] 2.3 `ClosureCompletenessTests` (`@SVCs({"SVC_GW_0165"})`): each tampering
...
- [x] 2.4 Existing suites unmodified: `ApprovalTests` (SVC_GW_0005, SVC_GW_0009),
...
- [x] 3.3 `SnapshotClosureRepository`: `record` (called inside the snapshot
      ...
      resolvedSha)`; `@Requirements({"GW_0164"})`
...
- [x] 5.1 `ClosureCompletenessGate` and `ClosureIncompleteException`; the gate
      ...
      refusal is written to the ledger; `@Requirements({"GW_0165"})`
