...
- **Two requirements**. GW_VETTING_0030 was the highest id on `main` and nothing under
  `openspec/changes/` reserved above it, so this change first claimed GW_AUTH_0029 and
  GW_INGEST_0030 — and then renumbered to **GW_INGEST_0030 and GW_APPROVAL_0013**, because
  [#283](https://github.com/skillsgateway/skillsgateway/pull/283) had claimed
  GW_AUTH_0029 in a worktree neither branch could see and was opened first. Checking
  ...
  - **GW_INGEST_0030 — The resolved closure is recorded as an immutable domain object of
  ...
  - **GW_APPROVAL_0013 — Approval requires a complete closure.** `doApprove` refuses, with a
...
- `marketplace-ingestion` — GW_INGEST_0030 added.
- `snapshot-approval` — GW_APPROVAL_0013 added.
