...
- [x] 1.1 Confirm from the code, not from the issue, that `ref` is the only wrong
...
- [x] 1.2 Settle the issue's open question: does adoption derive from `ref`?
      Enumerate every `FetchLogRepository` query and its grouping columns.
      Answer belongs in `design.md` whichever way it comes out.
- [x] 1.3 Write the failing real-client test: approve snapshot A, approve
...
- [x] 3.1 Add a package-private static resolution function to
...
- [x] 4.1 Unit-test the resolution function directly: the ambiguous case
...
- [x] 5.1 `docs/manual/reference/api/audit.md` — the `ref` field row, the
...
- [x] 5.2 `docs/manual/reference/git-facade.md` — the auditing section states
      what an `upload-pack` entry names and the tip-wins rule.
- [x] 5.3 `docs/manual/concepts/snapshots-and-ledger.md` — the `ref` row of the
      ledger entry table.
...
- [x] 6.1 One fresh run of every gate after the last edit; paste real tails into
      ...
      the commit SHA.
- [x] 6.2 Archive the change as the final commit of the PR.
