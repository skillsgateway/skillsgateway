...
- [x] 1.1 Add GW_INGEST_0022 (inventory diff of a snapshot against the marketplace's
...
- [x] 1.2 Add SVC_GW_INGEST_0022 (GIVEN/WHEN/THEN) to
      `docs/reqstool/software_verification_cases.yml`

## 2. Backend (SVC_GW_INGEST_0022)
...
- [x] 2.2 `SnapshotContentService`: `ContentDiff` / `PluginDiff` / `SkillDiff` /
      ...
      `@Requirements({"GW_INGEST_0022"})` — tree-object comparison per skill directory,
...
- [x] 3.1 `ContentDiffTests` extends `AbstractGatewayTest`,
      `@SVCs({"SVC_GW_INGEST_0022"})`: first snapshot has no baseline and reports
...
- [x] 4.2 `components/snapshot-content-diff.tsx`: loading / error / no-baseline /
      no-changes / changes states, JSDoc `@Requirements GW_INGEST_0022`
...
- [x] 6.3 `reqstool status local -p docs/reqstool` — GW_INGEST_0022 complete; the run
