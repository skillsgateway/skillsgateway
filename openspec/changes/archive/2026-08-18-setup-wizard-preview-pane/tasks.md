...
- [x] 1.1 Add GW_AUTH_0014–GW_APPROVAL_0005 to `docs/reqstool/requirements.yml`
      (setup wizard; snapshot file inspection; diff vs served baseline;
      preview pane). GW_APPROVAL_0004/0074 and GW_OBSERVABILITY_0001–0078 are taken by other
      in-flight changes.
- [x] 1.2 Add SVC_GW_AUTH_0014–SVC_GW_APPROVAL_0005 to
      `docs/reqstool/software_verification_cases.yml`.

## 2. Backend

- [x] 2.1 `preview/SnapshotPreviewService`: tree listing (path, size, binary;
      ...
      when nothing is served). `@Requirements` GW_INGEST_0015, GW_INGEST_0016.
- [x] 2.2 `preview/SnapshotPreviewController`: the three GET routes, each with
...
- [x] 4.1 `PreviewTests` (SVC_GW_INGEST_0015): tree lists the pinned commit's paths;
...
- [x] 4.2 `PreviewTests` (SVC_GW_INGEST_0016): approve, advance upstream
...
- [x] 5.1 Queries `useSnapshotFiles` / `useSnapshotFile` / `useSnapshotDiff`;
      typed MSW handlers.
- [x] 5.2 `components/markdown-view.tsx`: inert React-element Markdown subset,
      zero `dangerouslySetInnerHTML`; component test proving embedded HTML
      renders as text.
- [x] 5.3 `components/snapshot-preview.tsx`: file tree, file viewer
      ...
      GW_INGEST_0015–GW_APPROVAL_0005; component tests.
- [x] 5.4 `components/setup-wizard.tsx` on the marketplace detail page:
      ...
      JSDoc `@Requirements` GW_AUTH_0014; component tests.
- [x] 5.5 Playwright e2e: wizard spec (SVC_GW_AUTH_0014) and preview/diff spec
      (SVC_GW_APPROVAL_0005, real held snapshot vs served baseline via the
      `E2E_PREVIEW_UPSTREAM_DIR` fixture).
- [x] 5.6 `/impeccable audit` + `harden` on the changed surface; findings
      fixed or dismissed in the PR body.

## 6. Documentation

- [x] 6.1 `reference/api/marketplaces.md`: the three preview endpoints.
- [x] 6.2 `reference/portal.md`: setup wizard and preview pane sections.
- [x] 6.3 `guides/consuming-skills.md`: the wizard as the paved path;
