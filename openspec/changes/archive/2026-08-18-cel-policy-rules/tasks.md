...
- [x] 1.1 GW_APPROVAL_0006–GW_APPROVAL_0009 in `docs/reqstool/requirements.yml`.
- [x] 1.2 SVC_GW_APPROVAL_0006–SVC_GW_APPROVAL_0009 in
...
- [x] 3.1 `policy/CelPolicy`: the declared variable environment (snapshot,
      ...
      iteration cap (SVC_GW_APPROVAL_0006, SVC_GW_APPROVAL_0007).
- [x] 3.2 `policy/SnapshotFactsService` (+ pure `policy/SkillFrontmatter`):
      ...
      frontmatter is a build failure) (SVC_GW_APPROVAL_0007, `@Requirements` on the
...
- [x] 4.2 `policy/PolicyRuleService`: create/update/delete/list with name
      ...
      and reconciler (SVC_GW_APPROVAL_0006, `@Requirements({"GW_APPROVAL_0006"})`).
...
- [x] 6.1 `policy/PolicyController` (`/api/policy`): POST/GET `rules`,
      ...
      `Policy` tag in `api/OpenAPI.java` (SVC_GW_APPROVAL_0006, SVC_GW_APPROVAL_0009).
...
- [x] 8.2 `PolicyRuleApiTests` (`AbstractGatewayTest`): CRUD happy paths,
      ...
      rule 404 (SVC_GW_APPROVAL_0006, `@SVCs`).
- [x] 8.3 `PolicyEnforcementTests`: matching rule denies with 409 naming
      ...
      entries per deciding rule (SVC_GW_APPROVAL_0007, SVC_GW_APPROVAL_0008).
- [x] 8.4 `PolicyPlaygroundTests`: matched/false/error answers against a
      ...
      tip before/after, including error paths (SVC_GW_APPROVAL_0009).
