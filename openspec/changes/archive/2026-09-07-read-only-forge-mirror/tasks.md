...
- [x] 1.1 Add GW_FACADE_0020 (read-only forge mirror of approved content), GW_FACADE_0021 (the
      mirror is never an enforcement path), GW_FACADE_0022 (revocation propagates to the
      mirror) and GW_FACADE_0023 (mirror drift is reportable) to
      `docs/reqstool/requirements.yml`
- [x] 1.2 Add SVC_GW_FACADE_0020, SVC_GW_FACADE_0021, SVC_GW_FACADE_0022 and SVC_GW_FACADE_0023
...
- [x] 2.2 `MirrorUrlPolicy`: the same scheme allowlist registration applies, plus
      a refusal of a URL that embeds a credential (GW_FACADE_0020)
- [x] 2.3 `MirrorTarget.from`, validating at startup and refusing to start on an
      enabled-but-unusable configuration; nothing is contacted (GW_FACADE_0020)
...
- [x] 3.2 `ServedContentChangedEvent`, raised by `ApprovalService` after
      ...
      unpublish failed (GW_FACADE_0020, GW_FACADE_0022)
- [x] 3.3 `MirrorPublicationListener` and `MirrorConfiguration`'s single daemon
      thread: the wiring that keeps the mirror off the approving thread (GW_FACADE_0021)
...
- [x] 5.1 `MirrorUrlPolicyTests` (`@SVCs({"SVC_GW_FACADE_0020"})`): the scheme allowlist,
      an address with no usable scheme, a credential embedded in the URL, and an
      `@` in a path that is not one
- [x] 5.2 `ForgeMirrorTests` (`@SVCs({"SVC_GW_FACADE_0020", "SVC_GW_FACADE_0022",
      "SVC_GW_FACADE_0023"})`): against a real bare repository over `file://` —
...
- [x] 5.3 `ForgeMirrorFailureTests` (`@SVCs({"SVC_GW_FACADE_0021", "SVC_GW_FACADE_0022",
      "SVC_GW_FACADE_0023"})`): the mirror pointed at a path that is not a repository —
...
- [x] 5.4 `ForgeMirrorDisabledTests` (`@SVCs({"SVC_GW_FACADE_0020"})`): the shared
