...
<!-- Deviations from the plan as written, so a reader is not left to diff it:
     ...
     * 2.1 - 2.5 ran red first against throwing stubs. 2.6 and section 4/6 did
       ...
       implemented before their integration suite existed. Five of the fifteen
       mutants target exactly that code, and evidence.md records the deviation
       rather than implying the loop was followed.
     * 5.2's post-condition became the *single* enforcement point rather than a
       second one. Mutation testing showed that with the rewriter also comparing
       admitted-source names against graft names, dropping the post-condition
       changed nothing any test could see — so the name comparison lost that
       half and an unresolved source is now left as declared, where the gate can
       see it.
     * 8 gained a mutation layer (`mutants.sh`) that the plan did not name, and
       records changed-line coverage as unavailable: the project configures no
       coverage tool.
-->
...
- [x] 2.6 `ExternalSourceResolutionTests` (`@SVCs({"SVC_GW_0155", "SVC_GW_0156",
      ...
      declared `sha`
- [x] 2.7 `GitHttpFixture`: an in-process smart-HTTP git server over
...
- [x] 4.1 `SkillsGatewayProperties.ExternalSources`: `githubBaseUrl`,
...
- [x] 5.1 `ManifestRewriter`: in-core `DirCache` composition of the upstream tree
...
- [x] 7.1 `docs/manual/reference/configuration.md`: the new
      `external-sources.*` keys, the budgets block, and the hardening note that
      egress isolation remains the primary control
- [x] 7.2 `docs/manual/concepts/trust-boundaries.md`: the outbound path, the
      two-tier address policy, the redirect rules and what is deliberately not
      claimed
- [x] 7.3 `docs/manual/concepts/snapshots-and-ledger.md`: the composite SHA, the
      upstream commit as its parent, and how to diff the transformation
- [x] 7.4 `docs/manual/architecture.md`: the ingestion component and the Phase 2
      roadmap entry
- [x] 7.5 `docs/manual/reference/compatibility.md` (the source-form table) and
      `docs/manual/guides/approving-snapshots.md` (what a reviewer sees for an
      external plugin, and what each new violation means)
- [x] 7.6 `docs/decisions/0011-external-plugin-sources.md`: the increment table's
...
- [x] 8.6 `evidence.md`: the spec ↔ test mapping, the adversarial cases with
