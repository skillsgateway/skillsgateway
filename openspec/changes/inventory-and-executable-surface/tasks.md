...
- [ ] 2.1 `PluginComponentsTests` (pure, no Spring). Cover default locations;
  ...
  against a stub.
- [ ] 2.2 `PluginComponents` in `ingestion/`, annotated
  `@Requirements GW_INGEST_0045`. Make 2.1 green.
- [ ] 2.3 `SnapshotContentService.PluginContent` gains `commands`, `agents`,
  ...
  `SnapshotComponentsTests` (`@SVCs SVC_GW_INGEST_0045`) through
  `GET /api/v1/snapshots/{id}/content`.
- [ ] 2.4 Regenerate `openapi.json` and `types.gen.ts`. Verify that
  `OpenApiContractTests` passes.

## 3. The executable-surface vetter (SVC_GW_VETTING_0047–0049)

- [ ] 3.1 `ExecutableSurfaceVetterTests` over `InMemorySnapshot`. Cover:
...
- [ ] 3.2 `ExecutableSurfaceVetter` (order 250, version 1),
  `@Requirements GW_VETTING_0047/0048/0049`, and the name reserved in
  `ExternalConnectorProperties`. Make 3.1 green.
- [ ] 3.3 Integration test `ExecutableSurfaceTests` (`@SVCs` 0047, 0048,
...
- [ ] 3.6 Measure against `pbakaus/impeccable` at a pinned SHA and record the
