...
- [ ] 1.1 Add GW_0075–GW_0078 to `docs/reqstool/requirements.yml`
...
- [ ] 1.2 Add SVC_GW_0075–SVC_GW_0078 to
      `docs/reqstool/software_verification_cases.yml`.

## 2. Backend

- [ ] 2.1 `FetchLogRepository`: windowed `upload-pack` aggregation grouped by
      (marketplace, sha) and the latest-fetch-per-(principal, marketplace)
      query; covering partial index in `V1__init.sql`.
- [ ] 2.2 `adoption/AdoptionService`: fold aggregation rows into
      ...
      GW_0076.
- [ ] 2.3 `adoption/AdoptionController`: `GET /api/adoption?days=`,
      `GET /api/adoption/staleness`, both `requireAuditor` first line,
      OpenAPI annotations, `@Schema` on DTOs.
- [ ] 2.4 `observability/GatewayMetrics`: `skills_gateway.ingestion` and
...
- [ ] 4.1 Adoption (SVC_GW_0075): two identities fetch two marketplaces
...
- [ ] 4.2 Staleness (SVC_GW_0076): an identity that fetched a superseded SHA
...
- [ ] 4.3 Observability (SVC_GW_0077): with the default (no export) config,
...
- [ ] 4.4 RoleEnforcementTests: the two new reads added to PRIVILEGED_READS.

## 5. Portal

- [ ] 5.1 `useAdoption(days)` / `useStaleness()` queries; MSW handlers typed
      from the regenerated types.
- [ ] 5.2 `pages/adoption.tsx`: stat chips, per-marketplace row cards with
...
- [ ] 5.3 Playwright e2e (SVC_GW_0078, snake_case title): mint a PAT in the
      portal, clone through the facade, see the fetch and identity on the
      Adoption page.
- [ ] 5.4 `/impeccable audit` + `harden` + `critique` on the new page;
      findings fixed or dismissed in the PR body.

## 6. Documentation

- [ ] 6.1 New `docs/manual/reference/api/adoption.md`; nav entry.
- [ ] 6.2 New `docs/manual/reference/observability.md` (metric names, how a
      deployment enables export, the local `observability` profile); nav.
- [ ] 6.3 `reference/portal.md`: the Adoption page.
