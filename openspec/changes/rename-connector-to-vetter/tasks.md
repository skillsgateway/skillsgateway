...
- [ ] 1.1 `VettingConnector` → `Vetter`; `SecretScanConnector`,
      `PromptInjectionConnector`, `LicenseScanConnector`,
      `SkillConformanceConnector` → `*Vetter`
- [ ] 1.2 `ConnectorToggle`, `ConnectorToggleRepository`,
...
- [ ] 1.3 Every field, parameter, method, javadoc, log message, `@Operation`
      text, `@Schema` description and error message with them
- [ ] 1.4 `ExternalVettingConnector`, `ExternalConnectorProperties`,
...
- [ ] 1.5 Ledger events `vetter-disabled` / `vetter-enabled`; the
      `vetting-verdict` detail leads `vetter=state`; `revet-violation` carries
      `vetters=`
- [ ] 1.6 Finding rule ids `vetter-error` and `vetter-not-run` (not
      `vetter-disabled` — see design, Decision 3)
- [ ] 1.7 Test sources follow, including `ConnectorToggleTests` →
...
- [ ] 5.2 Delta specs for the seven capabilities whose requirements moved

## 6. Documentation

- [ ] 6.1 All of `docs/manual`; `vetting.md` retitled *Vetting — the vetter
      chain*, its *External connectors* section kept and reworded to "each
      contributes one vetter"
- [ ] 6.2 Glossary: a **Vetter** entry; **Connector** narrowed to the transport
      with the rule-of-thumb sentence
- [ ] 6.3 `reference/decisions.md`: dated renamed-on notes under ADR 0009,
      ADR 0010 and ADR 0015; `docs/decisions/*.md` untouched
- [ ] 6.4 `docs/diagrams/lifecycle.mmd` and its regenerated SVG pair, manifest
      hash refreshed
- [ ] 6.5 `PRODUCT.md`, `DESIGN.md`, `.claude/skills/architecture/SKILL.md`,
      `docs/manual/capability-map.md`

## 7. Gates

- [ ] 7.1 Run the gates and record `evidence.md`
- [ ] 7.2 `grep -rniw connector … | grep -vi external` is near-empty, and every
      remaining hit is justified in the PR notes
- [ ] 7.3 Archive this change as the PR's final commit
