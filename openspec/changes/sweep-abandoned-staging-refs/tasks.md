...
- [ ] 1.1 Add GW_0168 (an abandoned publication staging reference is removed and
...
- [ ] 1.2 Add SVC_GW_0168 (GIVEN/WHEN/THEN, both backends) to
      `docs/reqstool/software_verification_cases.yml`

## 2. Backend (SVC_GW_0168)

- [ ] 2.1 `V1__init.sql`: `staging_ref_sightings (marketplace, ref,
      first_seen_at)`, unique on `(marketplace, ref)`
- [ ] 2.2 `StagingRefSightingRepository`: `observe` (stamp the new, forget the
      gone, read back the stamps, one transaction) and `forget`
- [ ] 2.3 `SkillsGatewayProperties.Retention`: `stagingRefMaxAge` with a `24h`
      default and a `stagingSweepEnabled()` fail-safe on zero or negative
- [ ] 2.4 `RetentionService.sweepStagingRefs`: list `refs/staging/*` per
...
- [ ] 2.5 `RetentionService.compact` calls it, with its failures caught
      separately; `collectGarbage` becomes role-aware

## 3. Tests (never weakening an existing SVC test)

- [ ] 3.1 `StagingRefSweepTests`, `@SVCs({"SVC_GW_0168"})`, parameterized over
...
- [ ] 3.2 The filesystem case that the objects are physically reclaimed, not
      only unreferenced
- [ ] 3.3 The wiring: an on-demand compaction pass runs the sweep, records it on
      the ledger, and leaves approved content clonable through the facade
- [ ] 3.4 `staging-ref-max-age: 0` switches the sweep off rather than sweeping
      everything

## 4. Docs (same PR)

- [ ] 4.1 `docs/manual/reference/retention.md`: the sweep, its two conditions,
      the bound and what it protects — and the correction that retention now
      does touch the published repository
- [ ] 4.2 `docs/manual/reference/configuration.md`: `staging-ref-max-age` in the
      retention YAML sample and the property table
- [ ] 4.3 `docs/manual/guides/snapshot-retention.md`: what an operator sees
...
- [ ] 5.2 `openspec/changes/sweep-abandoned-staging-refs/evidence.md`
