...
- [ ] 1.1 Add GW_0145 (the contract document describes every lifecycle event, its
      payload and its transport headers, generated from the registry that produces
      the deliveries) and GW_0146 (a breaking change to the webhook event payload
      ...
      Ids start at GW_0145 deliberately; see proposal — Impact.
- [ ] 1.2 Add SVC_GW_0145 and SVC_GW_0146 to
      `docs/reqstool/software_verification_cases.yml`, GIVEN/WHEN/THEN in the
      style of SVC_GW_0106/SVC_GW_0107.
- [ ] 1.3 Confirm no other branch has since claimed GW_0145/GW_0146 (`grep`
...
- [ ] 2.1 `WebhookService.EventPayload`: add `requiredMode = Schema.RequiredMode.REQUIRED`
      ...
      `@Requirements({"GW_0145"})`.
- [ ] 2.2 Add a response record for the event registry (event names plus the
      ...
      `GW_0145`; keep `WebhookEvent.AUDIT_EXPORT` out by construction. Update the
      `@Operation` description to say the payload shape is included.
- [ ] 2.3 Confirm `EventPayload` now appears in `components.schemas` of the served
      document with a `required` array (it is absent today — nothing referenced it).

## 3. The `webhooks` object

- [ ] 3.1 Add an `OpenApiCustomizer` bean beside `documentVersion` in
      ...
      `@Requirements({"GW_0145"})`.
- [ ] 3.2 Describe the four transport headers on each operation as request headers
...
- [ ] 3.3 Add a `Webhooks` `@Tag` description so the entries render with the rest
      of the surface in Scalar.

## 4. Regenerate the published contract

- [ ] 4.1 `./mvnw test -Dtest=OpenApiDocsTests` then
      `cp target/openapi.json src/main/frontend/openapi.json`.
- [ ] 4.2 `(cd src/main/frontend && pnpm exec openapi-typescript openapi.json -o src/api/types.gen.ts)`.
- [ ] 4.3 Fix every portal caller of the events endpoint for the new response
      shape, and its MSW mock (ADR 0003: mocks are contract-derived, never
      hand-authored). `pnpm typecheck`.
- [ ] 4.4 Verify the committed document now carries a top-level `webhooks` object
      with all eight names and an `EventPayload` component.

## 5. Tests — and prove each one can fail

- [ ] 5.1 `OpenApiContractTests`: every name in `WebhookEvent.ALL` appears as a key
      ...
      `@SVCs({"SVC_GW_0145"})`.
- [ ] 5.2 `OpenApiContractTests`: `EventPayload` is a component schema whose
      ...
      depend on. Part of `SVC_GW_0145`.
- [ ] 5.3 `OpenApiContractTests`: each `webhooks` operation describes all four
      `X-Skills-Gateway-*` headers, asserted against the `WebhookSigner` constants
      rather than string literals. Part of `SVC_GW_0145`.
- [ ] 5.4 `OpenApiContractTests`: the events endpoint's `200` response schema
      reaches `EventPayload`, so the payload stays inside the `paths` surface the
      gate diffs with response severities. Part of `SVC_GW_0145`.
- [ ] 5.5 **Negative counterparts**, in the style of
...
- [ ] 5.6 `OpenApiContractTests`: extend
      ...
      apply to it identically. `@SVCs({"SVC_GW_0146"})`.
- [ ] 5.7 A dispatcher-side test that the serialised body actually carries every
      field the schema declares required — the contract is worthless if the
      document promises a field the wire omits. Part of `SVC_GW_0146`.
- [ ] 5.8 Confirm no existing SVC test was weakened or deleted to make any of the
      above pass (CLAUDE.md).

## 6. Write the promise down

- [ ] 6.1 `docs/manual/reference/compatibility.md` — extend "The API contract" so
...
- [ ] 6.2 `docs/manual/reference/compatibility.md` — "How it is enforced": say the
...
- [ ] 6.3 `docs/manual/guides/lifecycle-webhooks.md` — point receivers at the
...
- [ ] 6.4 `docs/manual/reference/api/index.md` — note that the reference now
      includes the webhook deliveries, not only the REST paths.
- [ ] 6.5 No requirement text anywhere in `docs/manual/` (CLAUDE.md — reqstool is
      SSOT); `mkdocs build --strict`.

## 7. Exercise the gate on this change

- [ ] 7.1 Label the PR `⚠️ BREAKING CONTRACT` and write a breaking conventional
...
- [ ] 7.2 Confirm the **API contract** check goes red without the label and green
      with it. Do not trust an unexercised gate.
- [ ] 7.3 On a scratch commit, delete an entry from `WebhookEvent.ALL`, regenerate
      the document, push, and confirm the check reports `webhook-removed` as an
      error. Revert the scratch commit.
- [ ] 7.4 On a scratch commit, rename one payload field, regenerate, push, and
      confirm the check reports `response-required-property-removed` as an error —
      the case the `webhooks` reference alone would only warn about. Revert.

## 8. Gates and close-out

- [ ] 8.1 `./mvnw clean verify` (`clean` — the reqstool gate needs it; needs
      Docker/Podman).
...
- [ ] 8.7 Write `openspec/changes/add-webhook-payload-contract/evidence.md`: the
...
- [ ] 8.8 Open the PR with an **Evidence** section summarising 8.7, and resolve
      design — Open Questions with the owner in the PR body.
- [ ] 8.9 Archive the change (`/opsx:archive`) as the final commit of the PR.
