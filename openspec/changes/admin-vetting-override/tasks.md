...
- [x] 1.1 Add GW_VETTING_0028 (admin override of a blocked vetting outcome: admin-only,
      reason-required, distinct ledger event, fail-loud marker, lifts only the
      vetting gate) and GW_VETTING_0029 (admin connector enable/disable, global or per
...
- [x] 1.2 Add SVC_GW_VETTING_0028 and SVC_GW_VETTING_0029 (GIVEN/WHEN/THEN) to
      `docs/reqstool/software_verification_cases.yml`

## 2. Failing tests first (old-coder: prove they fail)

- [x] 2.1 `VettingChainDisabledTests` (pure function, `@SVCs({"SVC_GW_VETTING_0029"})`):
...
- [x] 2.2 `ConnectorToggleTests` (extends `AbstractGatewayTest`,
      `@SVCs({"SVC_GW_VETTING_0029"})`): non-admin refused the toggle and the read; admin
...
- [x] 2.3 `VettingOverrideTests` (extends `AbstractGatewayTest`,
      `@SVCs({"SVC_GW_VETTING_0028"})`): a marketplace approver is refused the override
...
- [x] 4.2 `VettingChain.aggregate`: discount `DISABLED`, still require a clearing
      verdict; `@Requirements({"GW_VETTING_0002", "GW_VETTING_0029"})`
- [x] 4.3 `ConnectorToggleService` (resolution rule, validated audited `set`,
      ...
      verdict when off; `@Requirements({"GW_VETTING_0029"})`
...
- [x] 5.1 `ApprovalService.ApprovalOverride`; override path in `doApprove`
      ...
      `MissingOverrideReasonException`; `@Requirements({"GW_VETTING_0028"})`
