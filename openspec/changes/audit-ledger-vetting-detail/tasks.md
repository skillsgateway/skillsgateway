...
## 2. A clean pass records its coverage (GW_VETTING_0023)
...
- [x] 2.3 `SecretScanConnector`, `PromptInjectionConnector` and
      ...
      summary; annotate each `@Requirements({..., "GW_VETTING_0023"})`.
- [x] 2.4 Test: a snapshot that trips no rule yields a non-null, coverage-stating
      `detail` on each passing verdict and in the ledger (`SVC_GW_VETTING_0023`).

## 3. Verdict entries become self-describing and correctly attributed (GW_VETTING_0022)
...
- [x] 3.1 `VettingService.verdictDetail` builds `connector=state; findings=N;
      ...
      the `connector=state` lead so existing SVC_GW_VETTING_0006 substrings still match.
...
- [x] 3.4 Annotate the enrichment `@Requirements({"GW_VETTING_0006"})` and the actor
      classification path `@Requirements({..., "GW_AUDIT_0007"})`.
- [x] 3.5 Test: verdict entries carry the count, worst severity and run id; the
      ...
      (`SVC_GW_VETTING_0022`).
- [x] 3.6 Confirm the existing SVC_GW_VETTING_0006 ledger test still passes unchanged.

## 4. Requirements and traceability

- [x] 4.1 Author `GW_VETTING_0022`, `GW_VETTING_0023` in `docs/reqstool/requirements.yml`.
