...
- [ ] 1.1 Add GW_0159 (the approval-pending lifecycle event) and GW_0160 (its
      content-free payload) to `docs/reqstool/requirements.yml`
- [ ] 1.2 Add SVC_GW_0159 and SVC_GW_0160 to
      `docs/reqstool/software_verification_cases.yml`

## 2. Failing tests first (SVC_GW_0159, SVC_GW_0160)

- [ ] 2.1 In `WebhookTests`, add the event test: a subscriber filtering only
      ...
      (SVC_GW_0159)
- [ ] 2.2 In `WebhookTests`, add the adversarial payload test: the payload of a
...
- [ ] 2.3 Add the estate test: a subscriber declared in
...
- [ ] 2.4 Add the negative-state test: a re-vetting run against an **approved**
      snapshot emits `snapshot.revet_violation`/`snapshot.vetted` but never
      `snapshot.approval_pending` (SVC_GW_0159)
- [ ] 2.5 Run the new tests and record them failing before any production code
      changes

## 3. Implementation

- [ ] 3.1 `WebhookEvent`: add `SNAPSHOT_APPROVAL_PENDING` with its rationale
      comment and append it to `ALL`
- [ ] 3.2 `WebhookService`: add `ApprovalPendingPayload` and `VettingSummary`
...
- [ ] 3.3 `VettingService`: inject `WaiverService`, and after the existing
      `SNAPSHOT_VETTED` emit, emit the new event when the snapshot is held —
      annotated `@Requirements({"GW_0159"})`
- [ ] 3.4 Update the two test-side `new VettingService(...)` call sites for the
      new constructor argument

## 4. Documentation (same PR)

- [ ] 4.1 `docs/manual/guides/lifecycle-webhooks.md`: the new row in the event
...
- [ ] 4.2 Cross-link the approve/reject round-trip an external review system
...
- [ ] 5.4 `reqstool status local -p docs/reqstool` ends `PASS`
- [ ] 5.5 `openspec validate --all --strict`
- [ ] 5.6 `mkdocs build --strict`
- [ ] 5.7 Write `evidence.md` with the pasted tails of one final fresh run and
      the commit SHA
- [ ] 5.8 Archive the change as the final commit
