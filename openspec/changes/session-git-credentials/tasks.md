...
- [ ] 1.1 Add GW_0104 (a git credential derived from an authenticated browser
...
- [ ] 1.2 Add SVC_GW_0104 (GIVEN/WHEN/THEN) to
      `docs/reqstool/software_verification_cases.yml`

## 2. Tests first — observed failing before the implementation exists

- [ ] 2.1 `SessionCredentialTests` (extends `AbstractGatewayTest`),
...
- [ ] 2.2 Negative set, same `@SVCs`: a session credential cannot push to a
      hosted marketplace (F2); a narrowed one is refused another marketplace
      (F7); rotation keeps both the deadline and the mark (F4)
- [ ] 2.3 A context with a near-zero `session-ttl` asserts the credential fails
      authentication once past it, by comparison rather than by any sweep (F5)
- [ ] 2.4 Record the RED run

## 3. Schema, model and policy

- [ ] 3.1 `V1__init.sql`: `access_tokens.session_derived BOOLEAN NOT NULL
      DEFAULT FALSE`
- [ ] 3.2 `AccessToken` gains `sessionDerived`; `TokenRepository.create` and
      `map` carry it
- [ ] 3.3 `SkillsGatewayProperties.Tokens` gains `sessionTtl` (default 8h)

## 4. Issuing path

- [ ] 4.1 `TokenService.createSessionCredential(principal, name, scopes)`: the
      expiry is `now + sessionTtl`, no push scopes, `session_derived` set;
      `@Requirements({"GW_0104"})`
- [ ] 4.2 `TokenService.rotate` copies `session_derived` alongside the deadline
- [ ] 4.3 `TokenController`: `POST /api/tokens/session` with a body carrying a
      name and optional scopes and nothing else; audit detail says
      session-derived; `TokenView` gains `sessionDerived`

## 5. Portal types

- [ ] 5.1 Regenerate `openapi.json` → `types.gen.ts`

## 6. Documentation (same PR)

- [ ] 6.1 `docs/decisions/0008-serving-surface-stays-the-embedded-facade.md`
      (the #59 decision), indexed from `reference/decisions.md` and
      `architecture.md`
- [ ] 6.2 `guides/consuming-skills.md`: mint a session credential instead of a
      standing PAT, and what its TTL does and does not guarantee
- [ ] 6.3 `reference/api/tokens.md`, `reference/configuration.md`,
      `concepts/trust-boundaries.md`

## 7. Gates and evidence (old-coder gauntlet)

- [ ] 7.1 `./mvnw clean verify`
- [ ] 7.2 `(cd src/main/frontend && pnpm test:stories)` and `pnpm e2e`
- [ ] 7.3 `reqstool status local -p docs/reqstool` ends PASS;
      `openspec validate --all --strict`; `mkdocs build --strict`
- [ ] 7.4 Mutation pass over the issuing path with a negative control;
