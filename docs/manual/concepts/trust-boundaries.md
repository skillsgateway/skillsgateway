...
- A token may be **scoped** to named marketplaces (GW_AUTH_0006) and may **expire**
  (GW_AUTH_0007). Both are enforced where they matter: expiry at authentication (an
...
- Every fetch entry on the ledger names the token that authenticated it
  (GW_AUTH_0009), not just the principal: a principal with several tokens is several
  distinct credentials — and says whether that credential was derived from a
  browser session (GW_AUTH_0018) or deliberately provisioned, which is a different
